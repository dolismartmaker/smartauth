...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('smartauth', 'auth', 'write');
	$permissiontodelete = $user->hasRight('smartauth', 'auth', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
$title = $langs->trans("SmartauthArea");
//$help_url = "EN:Module_Auth|FR:Module_Auth_FR|ES:Módulo_Auth";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/smartauth/auth_card.php', 1).'?id='.((int) $id));
	exit;
}
...
// $newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/smartauth/auth_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
