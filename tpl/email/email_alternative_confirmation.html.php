<?php
/**
 * email_alternative_confirmation.html.php
 *
 * HTML body of the email sent when a connected user requests an
...
<head>
    <meta charset="UTF-8">
    <title>Confirmez votre adresse alternative</title>
</head>
        ...
        <tr>
            <td align="center">
                <table role="presentation" width="560" cellpadding="0" cellspacing="0" border="0" style="background:#ffffff;border-radius:8px;padding:32px;">
                    <?php if (!empty($serviceLogoUrl)): ?>
                    <tr>
                        <td align="center" style="padding-bottom:16px;">
                            <img src="<?= $h($serviceLogoUrl) ?>" alt="<?= $h($serviceName ?? '') ?>" style="max-height:64px;">
                        </td>
                    </tr>
                    <?php endif; ?>
                    <tr>
                        <td>
                            <h1 style="font-size:20px;margin:0 0 16px 0;color:#222;">Confirmez votre adresse alternative</h1>
                            <p style="margin:0 0 16px 0;line-height:1.5;">Bonjour,</p>
                            <p style="margin:0 0 16px 0;line-height:1.5;">
                                Vous avez demande d'utiliser <strong><?= $h($email ?? '') ?></strong>
                                comme adresse alternative
                                <?php if (!empty($serviceName)): ?>
                                    pour <strong><?= $h($serviceName) ?></strong>.
                                <?php else: ?>
                                    pour un service.
                                <?php endif; ?>
                            </p>
                            <p style="margin:0 0 24px 0;line-height:1.5;">
                                Cliquez sur le bouton ci-dessous pour confirmer (lien valable 24 heures).
                            </p>
