...
/**
 * \file        class/smartauthdevices.class.php
 * \ingroup     smartauth
 * \brief       This file is a CRUD class file for SmartAuthDevices (Create/Read/Update/Delete)
 */
...
class SmartAuthDevices extends \CommonObject
{
	 ...
	 */
	public $element = 'smartauthdevices';

	/**
	 * @var string Name of table without prefix where object is stored. This is also the key used for extrafields management.
	 */
	public $table_element = 'smartauth_devices';
	...
	/**
	 * @var string String with name of icon for smartauthdevices. Must be a 'fa-xxx' fontawesome code (or 'fa-xxx_fa_color_size') or 'smartauthdevices@smartauth' if picto is file 'img/object_smartauthdevices.png'.
	 */
	public $picto = 'fa-mobile';
	...
	//  */
	// public $table_element_line = 'smartauth_smartauthdevicesline';

	// /**
	//  * @var string    Field with ID of parent key if this object has a parent
	//  */
	// public $fk_element = 'fk_smartauthdevices';

	// /**
	//  * @var string    Name of subtable class that manage subtable lines
	//  */
	// public $class_element_line = 'SmartAuthDevicesline';

	// /**
	//  * @var array	List of child tables. To test if we can delete object.
	//  */
	// protected $childtables = array('mychildtable' => array('name'=>'SmartAuthDevices', 'fk_element'=>'fk_smartauthdevices'));
	...
	public function __construct(DoliDB $db)
	{
		...
		/*if ($user->hasRight('smartauth', 'smartauthdevices', 'read')) {
	 ...
	 */
	public function fetch($id, $ref = null, $uuid = null)
	{
		if (!empty($uuid)) {
			$sql = "SELECT rowid FROM " . MAIN_DB_PREFIX . $this->table_element;
			$sql .= " WHERE uuid='" . $this->db->escape($uuid) . "'";
	...
	public function validate($user, $notrigger = 0)
	{
		...
		/* if (! ((!getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && $user->hasRight('smartauth','write'))
		 || (getDolGlobalInt('MAIN_USE_ADVANCED_PERMS') && !empty($user->rights->smartauth->smartauthdevices->smartauthdevices_advance->validate))))
		 {
		...
		if (!$error) {
			...
			if (preg_match('/^[\(]?PROV/i', (string) $this->ref)) {
				...
				$dirsource = $conf->smartauth->dir_output . '/smartauthdevices/' . $oldref;
				$dirdest = $conf->smartauth->dir_output . '/smartauthdevices/' . $newref;
				if (!$error && file_exists($dirsource)) {
					...
					if (@rename($dirsource, $dirdest)) {
						...
						$listoffiles = dol_dir_list($conf->smartauth->dir_output . '/smartauthdevices/' . $newref, 'files', 1, '^' . preg_quote($oldref, '/'));
	...
	public function getTooltipContentArray($params)
	{
		...
		if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
			return ['optimize' => $langs->trans("ShowSmartAuthDevices")];
		}
		$datas['picto'] = img_picto('', $this->picto) . ' <u>' . $langs->trans("SmartAuthDevices") . '</u>';
	...
	public function getNomUrl($withpicto = 0, $option = '', $notooltip = 0, $morecss = '', $save_lastsearch_value = -1)
	{
		...
		$url = dol_buildpath('/smartauth/smartauthdevices_card.php', 1) . '?id=' . $this->id;
		...
		if (empty($notooltip)) {
			if (getDolGlobalInt('MAIN_OPTIMIZEFORTEXTBROWSER')) {
				$label = $langs->trans("ShowSmartAuthDevices");
	...
	public function getNextNumRef()
	{
		...
		if ($resql) {
			...
			dol_syslog("SmartAuth getNextValue", LOG_DEBUG);
			return -1;
		}
		...
		dol_syslog("SmartAuth getNextValue return " . $prefix . "-" . $num);
		return $prefix . "-" . $num;
	}
}
