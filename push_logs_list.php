...
/**
 *   	\file       push_logs_list.php
 *		\ingroup    smartauth
 *		\brief      List page for Web Push send logs
 */
...
$object = new SmartAuthPushLog($db);
...
// Permissions
$permissiontoread = $user->hasRight('smartauth', 'read');
$permissiontoadd = $user->hasRight('smartauth', 'write');
$permissiontodelete = $user->hasRight('smartauth', 'delete');
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'SmartAuthPushLog';
	$objectlabel = 'SmartAuthPushLog';
...
$title = $langs->trans("SmartAuthPushLogsTitle");
$help_url = '';
$morejs = array();
$morecss = array();
...
$topicmail = "";
$modelmail = "pushlogs";
$objecttmp = new SmartAuthPushLog($db);
$trackid = 'pushlog' . $object->id;
...
if ($search_all) {
	...
	print '<!-- Search done like if PUSHLOG_QUICKSEARCH_ON_FIELDS = ' . $setupstring . ' -->' . "\n";
...
foreach ($object->fields as $key => $val) {
	...
	} elseif (in_array($val['type'], array('double(24,8)', 'double(6,3)', 'integer', 'real', 'price')) && !in_array($key, array('id', 'rowid', 'ref')) && $val['label'] != 'TechnicalID' && empty($val['arrayofkeyval'])) {
...
while ($i < $imaxinloop) {
	...
	foreach ($object->fields as $key => $val) {
		...
		if (in_array($val['type'], array('double(24,8)', 'double(6,3)', 'integer', 'real', 'price')) && !in_array($key, array('id', 'rowid', 'ref')) && empty($val['arrayofkeyval'])) {
			$cssforfield .= ($cssforfield ? ' ' : '') . 'right';
		}
